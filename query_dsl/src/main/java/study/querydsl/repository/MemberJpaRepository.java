...
//        return queryFactory.selectFrom(QMember.member).where(member.username.eq(username)).fetch();
    ...
    public List<MemberTeamDto> searchByBooleanBuilder(MemberSearchCondition condition) {
        ...
        return queryFactory.select(new QMemberTeamDto(member.id.as("memberId"), member.username , member.age , team.id.as("teamId"),team.name)).from(member).leftJoin(member.team, team).where(builder).fetch();
    }
